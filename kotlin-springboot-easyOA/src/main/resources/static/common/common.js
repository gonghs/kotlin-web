...
(function () {
    window.commonUtil = {};
    commonUtil.ajax = (function (params) {
        var pp = {
    ...
    commonUtil.ajaxSubmit = (function (form, params) {// form 表单ID. params ajax参数
        var pp = {
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                layer.open({
                    type: 1,
    ...
    window.CommonUtils = {
        ...
        ajax: function (url, data, dataType) {
            if (!CommonUtils.notNull(dataType)) {
                dataType = "html";
            }
            var html = '没有结果!';
            // 所以AJAX都必须使用com.ajax..这里经过再次封装,统一处理..同时继承ajax所有属性
            if (url.indexOf("?") > -1) {
                url = url + "&_t=" + new Date();
            } else {
                url = url + "?_t=" + new Date();
